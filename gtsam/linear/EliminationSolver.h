      ...
      case LinearSolverParams::SEQUENTIAL_QR:
        return params_.ordering
                   ? gfg.eliminateSequential(*params_.ordering, EliminateQR)
                         ->optimize()
                   : gfg.eliminateSequential(params_.orderingType, EliminateQR)
                         ->optimize();
      case LinearSolverParams::SEQUENTIAL_CHOLESKY:
        return params_.ordering
                   ? gfg.eliminateSequential(*params_.ordering, EliminatePreferCholesky)
                         ->optimize()
                   : gfg.eliminateSequential(params_.orderingType, EliminatePreferCholesky)
                         ->optimize();
      default:
