...
/*****************************************************************************/
std::shared_ptr<LinearSolver> IterativeSolver::CreateFromParameters(
    const LinearSolverParams &params) {
  if (!params.iterativeParams) {
    throw std::runtime_error(
        "NonlinearOptimizer::solve: cg parameter has to be assigned ...");
  } else if (auto pcg = std::dynamic_pointer_cast<PCGSolverParameters>(
                 params.iterativeParams)) {
    return std::make_shared<PCGSolver>(*pcg);
    ...
    return std::make_shared<SubgraphSolverWrapper>(*spcg, *params.ordering);
  } else {
    throw std::runtime_error(
